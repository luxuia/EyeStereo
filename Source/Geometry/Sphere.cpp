...
using namespace EyeStereo;
	...
	void Sphere::setColor(D3DXCOLOR color) {
		mDiffColor = color;
	}
	...
	void Sphere::buildStacks(std::vector<Vertex14> & vertices, std::vector<DWORD> & indices){
	...
	bool Sphere::CreateMesh(const D3D10_INPUT_ELEMENT_DESC* layout) {
	...
	ID3DX10Mesh* Sphere::GetD3D10Mesh() {
		if (mMesh == NULL) {
