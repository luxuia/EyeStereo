...
void InitApp()
{
	...
	g_pChooseItRender = new ChooseItRender();
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if ( g_pChooseItRender->bactive == true ) {
		g_pChooseItRender->stereoRender(fElapsedTime);
		flgStereo = true;
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_CHOOSEIT_SWITCH: {
			...
			bool flg = g_pChooseItRender->bactive;
			flg = !flg;
