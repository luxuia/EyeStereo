...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_MAZE,	L"走迷宫",		35, iY+=24, 125, 22);
                                      ...
                                      void* pUserContext )
{
	...
	//g_pStereoSetting->makeBigTexture(pd3dDevice);
 	...
 	g_pMaze->init(pd3dDevice, g_pStereoSetting, &g_Camera);
 	g_pMaze->ShareMessage(g_pShareMessage, &g_bShareChanged);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pMaze->bactive == true ) {
		g_pMaze->stereoRender(fElapsedTime);
	}
                          ...
                          void* pUserContext )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (g_pMaze->bactive) {
		g_pMaze->HandMessages(uMsg, lParam);
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		/*case IDC_BRIGHTNESS_LEFT: {
			g_pMaze->g_brightness_left = g_SampleUI.GetSlider( IDC_BRIGHTNESS_LEFT )->GetValue()/100.0;
			break;
		}

		case IDC_BRIGHTNESS_RIGHT: {
			g_pMaze->g_brightness_right = g_SampleUI.GetSlider( IDC_BRIGHTNESS_RIGHT )->GetValue()/100.0;
			break;
		}
		case IDC_CONSTRACT: {
			g_pMaze->g_contrast = g_SampleUI.GetSlider( IDC_CONSTRACT )->GetValue()/100.0;
			break;
		}*/
        case IDC_TOGGLEWARP:
		...
		case IDC_MAZE: {
			g_pMaze->bactive = !g_pMaze->bactive;

			if (g_pMaze->bactive) {
				g_HUD.GetButton( IDC_MAZE )->SetText(L"结束迷宫");
			} else {
				g_HUD.GetButton( IDC_MAZE )->SetText(L"开始迷宫");
			}
			break;
		}
		case IDC_STEREO_OPEN: {

			g_pMaze->bStereo = !g_pMaze->bStereo;
			if (g_pMaze->bStereo) {
