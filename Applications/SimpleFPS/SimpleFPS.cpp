...
bool SimpleFPS::init(ID3D10Device* pd3d, StereoSetting* pstereo, CModelViewerCamera *pcamera) {
	printf("%s init start... \n", __FILE__);
...
void SimpleFPS::createNewObj()
{
	...
	g_ObjectsSpeed.push_back(D3DXVECTOR3(-4, RandF(-1,1), 0));
	g_ObjectsPosition.push_back(D3DXVECTOR3(10, RandF(-5,5), RandF(5,20)));

}

void SimpleFPS::reset()
{
...
void SimpleFPS::logicUpdate(float fTime) 
{
...
void SimpleFPS::stereoRender(float fTime) {
...
void SimpleFPS::RenderSubset(UINT id, UINT iSubSet) {
	HRESULT hr;
...
*/
void SimpleFPS::shot(int sx, int sy) {
...
void SimpleFPS::HandMessages(UINT uMsg, LPARAM lParam) {
...
void SimpleFPS::ShareMessage(WCHAR* sharestr, bool *sharechanged) {
...
void SimpleFPS::ClearScreen(ID3D10Device *pd3dDevice)
{
