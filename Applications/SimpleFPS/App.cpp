...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_FPS_START, L"开始射击", 35, iY += 24, 125, 22);
                                      ...
                                      void* pUserContext )
{
	...
	//g_pStereoSetting->makeBigTexture(pd3dDevice);
 	...
 	g_pSimpleFPS->init(pd3dDevice, g_pStereoSetting, &g_Camera);
 	g_pSimpleFPS->ShareMessage(g_pShareMessage, &g_bShareChanged);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pSimpleFPS->bactive == true ) {

		g_pSimpleFPS->logicUpdate(fElapsedTime);

		g_pSimpleFPS->stereoRender(fElapsedTime);
	}
                          ...
                          void* pUserContext )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (g_pSimpleFPS->bactive) {
		g_pSimpleFPS->HandMessages(uMsg, lParam);
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_BRIGHTNESS_LEFT: {
			g_pSimpleFPS->g_brightness_left = g_SampleUI.GetSlider( IDC_BRIGHTNESS_LEFT )->GetValue()/100.0;
			break;
		}

		case IDC_BRIGHTNESS_RIGHT: {
									   g_pSimpleFPS->g_brightness_right = g_SampleUI.GetSlider(IDC_BRIGHTNESS_RIGHT)->GetValue() / 100.0;
									   break;
		}
			/*case IDC_CONSTRACT: {
		...
		case IDC_FPS_START: {
								 g_pSimpleFPS->bactive = !g_pSimpleFPS->bactive;

								 if (g_pSimpleFPS->bactive) {
									 g_HUD.GetButton(IDC_FPS_START)->SetText(L"结束射击");
								 }
								 else {
									 g_pSimpleFPS->reset();
									 g_HUD.GetButton(IDC_FPS_START)->SetText(L"开始射击");
								 }
								 break;
		}
		case IDC_STEREO_OPEN: {

								  g_pSimpleFPS->bStereo = !g_pSimpleFPS->bStereo;
								  if (g_pSimpleFPS->bStereo) {
