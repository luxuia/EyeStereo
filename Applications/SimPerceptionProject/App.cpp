...
SimPerception*				g_pSimPerception = NULL;
...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_SIMPERCEPTION_SWITCH, L"开始同时视测试", 35, iY += 24, 125, 22);
	...
	g_pSimPerception = new SimPerception();
                                      ...
                                      void* pUserContext )
{
	...
	g_pSimPerception->init(pd3dDevice, g_pStereoSetting);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pSimPerception->bactive == true) {
		g_pSimPerception->stereoRender();
		flgStereo = true;
	}
                          ...
                          void* pUserContext )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (g_pSimPerception->bactive) {
		//g_pSimPerception->HandMessages(hWnd, uMsg, wParam, lParam);
		if (uMsg == WM_LBUTTONUP) {
			// Log
			Log::Get().Write(1, 1, 1); // Should Be Score time CorrectPercent
		}
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_STEREO_OPEN: {
			...
			g_pSimPerception->bStereo = !g_pSimPerception->bStereo;
			if (g_pSimPerception->bStereo) {
		...
		case IDC_SIMPERCEPTION_SWITCH: {

			bool flg = g_pSimPerception->bactive;
			flg = !flg;
			g_pSimPerception->bactive = flg;

			if (g_pSimPerception->bactive) {
				g_HUD.GetButton(IDC_SIMPERCEPTION_SWITCH)->SetText(L"结束测试");
			} else {
				g_HUD.GetButton(IDC_SIMPERCEPTION_SWITCH)->SetText(L"开始同时视测试");
			}
			break;
		}
