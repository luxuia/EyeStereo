...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_FILLABLE,		L"FillAble",  35, iY+=24, 125, 22);
                                      ...
                                      void* pUserContext )
{
	...
	g_pFillAble->init(pd3dDevice, g_pStereoSetting, &g_Camera, g_pKeyBoard);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pFillAble->bactive == true) {
		g_pFillAble->stereoRender(fTime);
		flgStereo = true;
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_STEREO_OPEN: {
			...
			g_pFillAble->bStereo = !g_pFillAble->bStereo;
			if (g_pFillAble->bStereo) {
		...
		case IDC_FILLABLE: {
			g_pFillAble->bactive = !g_pFillAble->bactive;

			if (g_pFillAble->bactive) {
				g_HUD.GetButton( IDC_FILLABLE )->SetText(L"结束填充");
			} else {
				g_HUD.GetButton( IDC_FILLABLE )->SetText(L"开始填充");
				g_pFillAble->Restart();
			}
			break;
		}
