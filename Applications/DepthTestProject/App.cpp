...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_DEPTH_TEST,	L"深度测试",		35, iY+=24, 125, 22);
                                      ...
                                      void* pUserContext )
{
	...
	//g_pStereoSetting->makeBigTexture(pd3dDevice);
 	...
 	g_pDepthTrain->init(pd3dDevice, g_pStereoSetting, &g_Camera);
 	g_pDepthTrain->ShareMessage(g_pShareMessage, &g_bShareChanged);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pDepthTrain->bactive == true ) {
		g_pDepthTrain->stereoRender(fElapsedTime);
	}
                          ...
                          void* pUserContext )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (g_pDepthTrain->bactive) {
		g_pDepthTrain->HandMessages(uMsg, lParam);
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_BRIGHTNESS_LEFT: {
			g_pDepthTrain->g_brightness_left = g_SampleUI.GetSlider( IDC_BRIGHTNESS_LEFT )->GetValue()/100.0;
			break;
		}
		...
		case IDC_DEPTH_TEST: {
								 ...
								 if (g_pDepthTrain->bactive) {
									 g_HUD.GetButton(IDC_DEPTH_TEST)->SetText(L"结束深度测试");
								 }
								 else {
									 g_HUD.GetButton(IDC_DEPTH_TEST)->SetText(L"开始深度测试");
								 }
								 break;
		}
