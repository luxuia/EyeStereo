...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_LEMNISCUS,		L"双纽线",		35, iY+=24, 125, 22);
                                      ...
                                      void* pUserContext )
{
	...
	g_pLemniscus->init(pd3dDevice, g_pStereoSetting, &g_Camera);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if ( g_pLemniscus->bactive == true) {
		g_pLemniscus->stereoRender(fElapsedTime);
		flgStereo = true;
	}
    ...
    switch( nControlID )
    {
		...
		case IDC_LEMNISCUS: {
			...
			if (g_pLemniscus->bactive) {
				///begin Lemniscus app;
				g_pTimer->Reset();
				g_HUD.GetButton( IDC_LEMNISCUS)->SetText(L"结束双纽线"); 
			} else {
				...
				g_HUD.GetButton( IDC_LEMNISCUS)->SetText(L"开始双纽线");
			}
			break;
		}
