...
#include "Utility/PCH.h"  ///< Pre Compiled Header， 所有项目都必须要的包含的头文件
#include "Utility/Log.h"  ///< 日志模块
#include "Utility/Timer.h"  ///< 计时模块


#include "Stereo/StereoSetting.h"  ///< 立体视辅助
#include "Event/KeyBoardState.h"   ///< 键盘消息辅助
#include "Helper/WavAudioPlayer.h" ///< 音频接口辅助

#include "Basic.h"   ///< 自定义的模块
...
ChooseItRender*				g_pBasicRender = NULL;
...
enum {
 ...
 IDC_STEREO_OPEN,  ///< 打开Stereo效果
 IDC_CHOOSEIT_SWITCH,  ///< 打开App 

 IDC_STEREO_SEPARATION_SILDER, ///< 设定双眼目距 

 IDC_SHARE_MESSAGE,  ///< 用于提供消息提示显示 
...
void InitApp()
{
	...
	g_HUD.AddButton(IDC_CHOOSEIT_SWITCH, L"打开BasicRender", 35, iY += 24, 125, 22);
	...
	g_pBasicRender = new BasicRender();
...
//--------------------------------------------------------------------------------------
/**
    初始化 ID3D10Device 设备，camera信息和引用类
*/
HRESULT CALLBACK OnD3D10CreateDevice( ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc,
                                      void* pUserContext )
{
	...
	g_pBasicRender->init(pd3dDevice, g_pStereoSetting, &g_Camera);
...
//--------------------------------------------------------------------------------------
/**
    窗口大小变化时产生调用，使用 StereoSetting::makeBigTexture 重新生成Stereo纹理
*/
HRESULT CALLBACK OnD3D10ResizedSwapChain( ID3D10Device* pd3dDevice, IDXGISwapChain* pSwapChain,
                                          const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
...
//--------------------------------------------------------------------------------------
/**
    渲染函数，每一帧都会调用。输出一个两倍于窗口大小的图像，详见Stereo实现。
*/
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    
	static int ppppp = 0;
	if (ppppp  < 3) {ppppp++;return;};  //<  ppppp 变量用于缓冲，防止计算资源浪费在窗口未显示阶段。
	...
	if ( g_pBasicRender->bactive == true) {
		g_pBasicRender->stereoRender(fElapsedTime);   //< 模块正式渲染函数调用
		flgStereo = true;
	}
...
//--------------------------------------------------------------------------------------
/**
    关闭程序后清理资源，注意D3D是基于Windows COM设计，谁申请谁释放。  
*/
void CALLBACK OnD3D10DestroyDevice( void* pUserContext )
{
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_STEREO_OPEN: {
			...
			g_pBasicRender->bStereo = !g_pBasicRender->bStereo;
			if (g_pBasicRender->bStereo) {
		...
		case IDC_CHOOSEIT_SWITCH: {

			bool flg = g_pBasicRender->bactive;
			flg = !flg;
			g_pBasicRender->bactive = flg;

			if (g_pChooseItRender->bactive) {
				g_HUD.GetButton(IDC_CHOOSEIT_SWITCH)->SetText(L"结束测试");
			} else {
				g_HUD.GetButton(IDC_CHOOSEIT_SWITCH)->SetText(L"开始Basic");
			}
			break;
		}
    }
