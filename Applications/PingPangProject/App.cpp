...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_PINGPANG,		L"乒乓小游戏",	35, iY+=24, 125, 22);
	iY = 0;
                                      ...
                                      void* pUserContext )
{
	...
	g_pPingPang->init(pd3dDevice, g_pStereoSetting, &g_Camera, g_pKeyBoard);
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_STEREO_OPEN: {
			g_pPingPang->bstereo = !g_pPingPang->bstereo;
			if (g_pPingPang->bstereo) {
		...
		case IDC_PINGPANG: {
			g_pPingPang->bactive = !g_pPingPang->bactive;

			if (g_pPingPang->bactive == true) {
				g_pPingPang->Restart();
				g_HUD.GetButton( IDC_PINGPANG )->SetText(L"结束乒乓");
			} else {
				g_HUD.GetButton( IDC_PINGPANG )->SetText(L"重新开始");
