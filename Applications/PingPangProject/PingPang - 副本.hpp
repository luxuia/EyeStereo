...
class PingPang {
	...
	class blockBox : public DiffSpecularBox {
	public:
		enum LIVE_STATE {FOREVER, LIVE, DIE};
		LIVE_STATE live;
		
		blockBox(float ll, float hh, float ww, D3DXCOLOR diff, D3DXCOLOR spec = YELLOW) {
			live = LIVE;
			DiffSpecularBox(ll, hh, ww, diff, spec);
		}
	};
...
public:
	...
	Ball*							pBall;
	std::vector<blockBox*>			pBoxVector;			
