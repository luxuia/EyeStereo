...
MatchMaster*				g_pMatchMaster = NULL;
...
void InitApp()
{
	...
	g_HUD.AddButton( IDC_MATCH_MASTER,	L"连连看",		35, iY+=24, 125, 22);
	...
	g_pMatchMaster = new MatchMaster();
                                      ...
                                      void* pUserContext )
{
	...
	//g_pStereoSetting->makeBigTexture(pd3dDevice);
 	...
 	g_pMatchMaster->init(pd3dDevice, g_pStereoSetting, &g_Camera);
 	g_pMatchMaster->ShareMessage(g_pShareMessage, &g_bShareChanged);
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
	...
	if (g_pMatchMaster->bactive == true ) {
		g_pMatchMaster->stereoRender(fElapsedTime);
	}
                          ...
                          void* pUserContext )
{
    ...
    *pbNoFurtherProcessing = g_SampleUI.MsgProc( hWnd, uMsg, wParam, lParam );
	...
	if (g_pMatchMaster->bactive) {
		g_pMatchMaster->HandMessages(uMsg, lParam);
	}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
		...
		case IDC_BRIGHTNESS_LEFT: {
			g_pMatchMaster->g_brightness_left = g_SampleUI.GetSlider( IDC_BRIGHTNESS_LEFT )->GetValue()/100.0;
			break;
		}

		case IDC_BRIGHTNESS_RIGHT: {
			g_pMatchMaster->g_brightness_right = g_SampleUI.GetSlider( IDC_BRIGHTNESS_RIGHT )->GetValue()/100.0;
			break;
		}
		/*case IDC_CONSTRACT: {
			g_pMatchMaster->g_contrast = g_SampleUI.GetSlider( IDC_CONSTRACT )->GetValue()/100.0;
			break;
		}*/
        case IDC_TOGGLEWARP:
		...
		case IDC_MATCH_MASTER: {
			g_pMatchMaster->bactive = !g_pMatchMaster->bactive;

			if (g_pMatchMaster->bactive) {
				g_HUD.GetButton( IDC_MATCH_MASTER )->SetText(L"结束深度测试");
			} else {
				g_HUD.GetButton( IDC_MATCH_MASTER )->SetText(L"开始深度测试");
			}
			break;
		}
		case IDC_STEREO_OPEN: {

			g_pMatchMaster->bStereo = !g_pMatchMaster->bStereo;
			if (g_pMatchMaster->bStereo) {
